...
json_encode_string(A, State) when is_atom(A) ->
    ...
    case json_string_is_safe(L) of
        true ->
            [?Q, L, ?Q];
        false ->
            json_encode_string_unicode(mpr_xmerl_ucs:from_utf8(L), State, [?Q])
    end;
json_encode_string(B, State) when is_binary(B) ->
    case json_bin_is_safe(B) of
        true ->
            [?Q, B, ?Q];
        false ->
            json_encode_string_unicode(mpr_xmerl_ucs:from_utf8(B), State, [?Q])
    end;
    ...
    case B of
        ...
        <<_:O/binary, "\\u", C3, C2, C1, C0, Rest/binary>> ->
            ...
            if C > 16#D7FF, C < 16#DC00 ->
                ...
                [CodePoint] = mpr_xmerl_ucs:from_utf16be(<<C:16/big-unsigned-integer,
                D:16/big-unsigned-integer>>),
                Acc1 = lists:reverse(mpr_xmerl_ucs:to_utf8(CodePoint), Acc),
                tokenize_string(B, ?ADV_COL(S, 12), Acc1);
                true ->
                    Acc1 = lists:reverse(mpr_xmerl_ucs:to_utf8(C), Acc),
